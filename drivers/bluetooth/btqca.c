		...
		struct qca_btsoc_version ver, u8 rom_ver, u16 bid)
{
	...
	if (rom_ver != 0) {
		if (bid == 0x0 || bid == 0xffff)
			snprintf(fwname, max_size, "/*(DEBLOBBED)*/", stem, rom_ver, variant);
		else
			snprintf(fwname, max_size, "/*(DEBLOBBED)*/", stem, rom_ver,
						variant, prefix, bid);
	} else {
		if (bid == 0x0 || bid == 0xffff)
			snprintf(fwname, max_size, "/*(DEBLOBBED)*/", stem, variant);
		else
			snprintf(fwname, max_size, "/*(DEBLOBBED)*/", stem, variant, prefix, bid);
	}
}
		   ...
		   const char *firmware_name, const char *rampatch_name)
{
	...
	config.type = TLV_TYPE_PATCH;
	if (rampatch_name) {
		snprintf(config.fwname, sizeof(config.fwname), "/*(DEBLOBBED)*/", rampatch_name);
	} else {
		switch (soc_type) {
		case QCA_QCA2066:
			snprintf(config.fwname, sizeof(config.fwname),
	...
	if (firmware_name) {
		...
		if (qca_filename_has_extension(firmware_name)) {
			snprintf(config.fwname, sizeof(config.fwname), "/*(DEBLOBBED)*/", firmware_name);
		} else {
			qca_read_fw_board_id(hdev, &boardid);
